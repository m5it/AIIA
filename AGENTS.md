...
4. After pruning, the `"Plan is ready! Starting first task."` system anchor is removed (it's superseded by the build instruction). The `"Mode changed to BUILD."` anchor is **kept in its original position** (the start of the build phase) so it never becomes a stale instruction placed after the current task work. The persona and other system messages are otherwise left in place.
