...
- `ReplaceLine` — Replace specific line(s) in a file (params: `<fileName>`, `<fromLine>`, `<toLine>` optional, `<replacement>`, `<confirmed>`). Three-phase flow: preview → `confirmed=true` applies + shows a verification diff (whole-file backup saved to `/tmp`) → `confirmed=finalize` accepts or `confirmed=revert` restores from backup
