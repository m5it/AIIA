...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO news (`date`, `time`, ending, pageno, pagephoto, pagetext, pagetitle, photo, title, subtitle, code) VALUES (%s, %s, %s, %s %s %s %s %s %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE news SET `date`=%s, `time`=%s, ending=%s, pageno=%s, pagephoto=%s, pagetext=%s, pagetitle=%s, photo=%s, title=%s, subtitle=%s, code=%s WHERE post_id=%s",
...
mysql_select_db($database_sanmig, $sanmig);
$query_getPosts = sprintf("SELECT post_id, `date`, `time`, ending, pageno, pagephoto, pagetext, pagetitle, photo, title, subtitle, code FROM news WHERE post_id = %s", GetSQLValueString($colname_getPosts, "int"));
