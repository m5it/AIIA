...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO gallery_ads (`place`, `number`, photo, title, subtitle, date) VALUES (%s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE gallery_ads SET `place`=%s, `number`=%s, photo=%s, title=%s, subtitle=%s, date=%s WHERE post_id=%s",
  ...
  $updateGoTo = "manage_gallery_ads.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_sanmig, $sanmig);
$query_getPosts = sprintf("SELECT post_id, `place`, `number`, photo, title, subtitle, date FROM gallery_ads WHERE post_id = %s", GetSQLValueString($colname_getPosts, "int"));
...
<title>Update Gallery Ads</title>
<link href="../../styles/admin.css" rel="stylesheet" type="text/css" />
</head>

<body>
<h1>Update Gallery Ads</h1>
