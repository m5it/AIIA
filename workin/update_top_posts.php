...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO top_posts (`date`, `time`, ending, pageno, pagephoto, pagetext, pagetitle, photo, title, subtitle, code) VALUES (%s, %s, %s, %s %s %s %s %s %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE top_posts SET `date`=%s, `time`=%s, ending=%s, pageno=%s, pagephoto=%s, pagetext=%s, pagetitle=%s, photo=%s, title=%s, subtitle=%s, code=%s WHERE post_id=%s",
  ...
  $updateGoTo = "manage_top_posts.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_sanmig, $sanmig);
$query_getPosts = sprintf("SELECT post_id, `date`, `time`, ending, pageno, pagephoto, pagetext, pagetitle, photo, title, subtitle, code FROM top_posts WHERE post_id = %s", GetSQLValueString($colname_getPosts, "int"));
...
<title>Update Top Posts</title>
<link href="../../styles/admin.css" rel="stylesheet" type="text/css" />
</head>

<body>
<h1> &nbsp; Update Top Posts</h1>
<p>
