...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO restaurants_ads (`place`, `number`, photo, title, subtitle, notes, date) VALUES (%s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE restaurants_ads SET `place`=%s, `number`=%s, photo=%s, title=%s, subtitle=%s, notes=%s, date=%s WHERE post_id=%s",
  ...
  $updateGoTo = "manage_restaurant_ads.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
date FROM restaurants_ads WHERE post_id = %s", GetSQLValueString($colname_getPosts, "int"));
