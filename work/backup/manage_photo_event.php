...
require_once('../Connections/sanmig-29.10.16.php');
...
$query_getPosts = "SELECT post_id, `date`, `time`, code, title, updated FROM photo_event WHERE photo_event.`date`> '01-10-31' ORDER BY photo_event.`date` DESC, photo_event.`time` DESC";
...
//mysql_select_db($database_sanmig, $sanmig);
//$query_getPosts = "SELECT post_id, `date`, `time`, code, title, updated FROM photo_event WHERE photo_event.`date`> '01-10-31' ORDER BY photo_event.`date` DESC, photo_event.`time` DESC";
//$query_getPosts = "SELECT post_id, `date`, `time`, code, title, updated FROM photo_event WHERE date>=DATE(NOW()-INTERVAL 45 DAY) ORDER BY photo_event.`date` DESC, photo_event.`time` DESC";
$query_getPosts = "SELECT post_id, `date`, `time`, code, title, updated FROM photo_event WHERE date>=DATE(NOW()-INTERVAL 1 YEAR) ORDER BY photo_event.`date` DESC, photo_event.`time` DESC";

$query_limit_getPosts = sprintf("%s LIMIT %d, %d", $query_getPosts, $startRow_getPosts, $maxRows_getPosts);

$getPosts = $sanmig->query($query_limit_getPosts) or die(mysqli_error($sanmig));
$row_getPosts = mysqli_fetch_assoc($getPosts);

if (isset($_GET['totalRows_getPosts'])) {
  $totalRows_getPosts = $_GET['totalRows_getPosts'];
} else {
  $all_getPosts = $sanmig->query($query_getPosts);
...
<title>Photo Events Manage</title>
<link href="../../styles/admin.css" rel="stylesheet" type="text/css" />
</head>

<body>
<h2> &nbsp; Manage Photo Events</h2>
&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<a href="add_photo_event.php"><FONT FACE="TREBUCHET MS" SIZE="+2" COLOR="#1E90FF"><B>Add Photo Event</a></B></font></a>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<a href="add_onetime_event.php"><FONT FACE="TREBUCHET MS" SIZE="-1" COLOR="#000000"><B>Add Onetime Event</a></B></font></a>  <P>
...
<td><a href="update_photo_event.php?post_id=<?php echo $row_getPosts['post_id']; ?>">Edit</a></td>
      <td><a href="delete_photo_event.php?post_id=<?php echo $row_getPosts['post_id']; ?>">Delete</a></td>
    </tr>
