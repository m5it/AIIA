...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO sanmig (`date`, `time`, ending, type, title, cost, location, contact, website, coloron, coloroff, `description`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE sanmig SET `date`=%s, `time`=%s, ending=%s, type=%s, title=%s, cost=%s, location=%s, contact=%s, website=%s, coloron=%s, coloroff=%s, `description`=%s WHERE post_id=%s",
                       GetSQLValueString($_POST['date'], "date"),
                       GetSQLValueString($_POST['time'], "date"),
...
mysql_select_db($database_sanmig, $sanmig);
$query_getPosts = sprintf("SELECT post_id, `date`, `time`, ending, type, title, location, contact, website, cost, coloron, coloroff, `description` FROM sanmig WHERE post_id = %s", GetSQLValueString($colname_getPosts, "int"));
