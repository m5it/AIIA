...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO ads2 (`place`, `number`, photo, title, subtitle, date) VALUES (%s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE ads2 SET `place`=%s, `number`=%s, photo=%s, title=%s, subtitle=%s, date=%s WHERE post_id=%s",
...
mysql_select_db($database_sanmig, $sanmig);
$query_getPosts = sprintf("SELECT post_id, `place`, `number`, photo, title, subtitle, date FROM ads2 WHERE post_id = %s", GetSQLValueString($colname_getPosts, "int"));
