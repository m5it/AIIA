...
class HandleContext():
	...
	def _pb_autoclean(self):
		...
		for idx, m in enumerate(new_msgs):
			if not isinstance(m, dict) or m.get('role') != 'system':
				continue
			c = (m.get('content') or '')
			if c.startswith('Plan is ready! Starting first task.'):
				plan_ready_idx = idx
			elif c.startswith('Mode changed to BUILD.'):
				build_switch_idx = idx
		# In build mode, the plan-ready anchor is superseded by the build
	...
	def _insert_summary(self, msgs, keep, summary):
		...
		Transient state updates such as "Mode changed to BUILD." and
		"Plan is ready! Starting first task." are not standing instructions.
		They are captured inside the summary so the model still sees the
		current task, but they are not kept as separate system messages that
		would duplicate or contradict the standing prompt."""
		...
		# Transient system messages generated by mode/plan transitions.
		TRANSIENT_PREFIXES = (
			'Mode changed to',
			'Plan is ready! Starting first task.',
		)

		# Collect existing summary texts and the latest transient state.
		chunks = []
		has_existing = False
		transient_state = []
		for m in new_msgs:
			if isinstance(m, dict) and m.get('role') == 'system':
				...
				if c.startswith('[Context summary:'):
					...
					has_existing = True
				elif any(c.startswith(p) for p in TRANSIENT_PREFIXES):
					transient_state.append(c)
		chunks.append(summary)  # newest last

		# Include the latest transient state in the summary so current mode/task
		# is not lost, but keep it concise.
		if transient_state:
			chunks.append(transient_state[-1])
		...
		# Drop old summary rows and transient state rows; keep everything else.
		kept = [
			m for m in new_msgs
			if not (isinstance(m, dict) and m.get('role') == 'system'
					and ((m.get('content', '') or '').startswith('[Context summary:')
						 or (m.get('content', '') or '').startswith(TRANSIENT_PREFIXES)))
		]
		...
		# Renumber rowIds so no duplicate/out-of-order IDs remain after history
		# is rewritten.  Order is preserved; only the display id is reset.
		for i, m in enumerate(kept):
