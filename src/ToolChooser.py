...
class ToolChooser():
	...
	#
	def Choose(self, auto_load_all=False):
		...
		self.Available()
		#
		# Auto-load all tools if requested (default behavior now)
		if auto_load_all or len(self.available) > 0:
			self.handle.hLG.echo("Auto-loading all available tools...",{'color':True,'colorValue':'green'})
			for tool_file in self.available:
				r = splitFileNameExtension(tool_file)
				if r['name'] not in self.selected:
					self.selected.append( r['name'] )
		#
		# Manual selection (optional)
		if not auto_load_all:
