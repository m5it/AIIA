...
def Run():
	...
	for i in range(len(texts)):
		...
		#print("i: {} / {} t( {} ): {}, inputs: {}".format( i, len(texts), labl, len(text), inputs ))
		...
		# Make a prediction
		#output = model(inputs)
		output = model(in_ids, attention_mask=in_ams, labels=torch.tensor([labl]).unsqueeze(0))
